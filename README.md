...
- PASS 비율·평균 준비도·채널 미리보기 준비 대시보드
- 플랫폼 수수료·광고·반품·결제·목표마진 정책 설정
- 반올림 방식·최저 판매가 시뮬레이션
- 브라우저 로컬 가격정책 저장
- 문제 필드 인라인 수정과 즉시 재진단
- 반응형·인쇄용 결과 화면
- JSON·HTML·Issues CSV·Payload JSON 결과
- 실행 비교 API

현재 단계와 다음 개발 순서는 [`docs/UPGRADE_ROADMAP.md`](docs/UPGRADE_ROADMAP.md)를 참고합니다.
...
7. 등록 전 진단에서 가격정책을 설정하고 CSV를 진단합니다.
8. 차단된 필드는 결과 표의 `화면에서 수정`으로 고친 뒤 즉시 재진단합니다.
