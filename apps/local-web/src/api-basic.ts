...
import { defaultPolicy, demoCsv, metadata, store } from "./context.ts";
import { readJsonBody, sendJson, sendText } from "./http-utils.ts";

export async function handleBasicApi(
  req: IncomingMessage,
  res: ServerResponse,
  url: URL
): Promise<boolean> {
  ...
  if (path === "/health/ready") {
    sendJson(res, 200, { status: "ready", metadata: metadata.version });
    return true;
  }
  if (path === "/api/demo.csv") {
    sendText(res, 200, demoCsv, "text/csv; charset=utf-8");
    return true;
  }
  ...
  if (path === "/api/diagnose" && req.method === "POST") {
    ...
    if (typeof input.csvText !== "string" || !input.csvText.trim()) {
      throw new Error("CSV 파일 내용이 필요합니다.");
    }
    const policy = {
      ...defaultPolicy,
      ...(input.pricePolicy ?? {})
    ...
    const report = diagnoseCsv({
      ...
      pricePolicy: policy,
      clock: systemClock(),
      maxRows: Number(process.env.MAX_INPUT_ROWS ?? 5000)
    });
  ...
  if (segments.length === 3 && req.method === "GET") {
    sendJson(res, 200, found.report);
    return true;
  }
  if (segments.length === 3 && req.method === "DELETE") {
    store.delete(segments[2]);
    sendJson(res, 200, { deleted: true });
    return true;
  }
  if (segments[3] === "patch" && req.method === "POST") {
    const input = await readJsonBody(req);
    const csv = patchSourceRow(
      found.report,
      Number(input.itemIndex),
      String(input.field),
      String(input.value)
    );
    const storedPolicy = found.report.run.pricePolicy ?? defaultPolicy;
    const report = diagnoseCsv({
      csvText: csv,
      filename: found.report.run.inputFilename,
      metadata,
      pricePolicy: storedPolicy,
      clock: systemClock()
    });
